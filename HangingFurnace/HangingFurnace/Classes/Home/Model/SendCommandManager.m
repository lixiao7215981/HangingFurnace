...
+(void)sendModeCmd:(SkywareDeviceInfoModel *)skywareInfo
{
    ...
    if (instance.deviceFunState == heating_fun) { //采暖
        if (instance.heating_select_model == ceaseless_run) { //无定时模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"21FF"];
        }else if (instance.heating_select_model == business_one){ //商务模式1
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"22FF"];
        }else if (instance.heating_select_model == business_two){ //商务模式2
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"23FF"];
        }else if (instance.heating_select_model == economy){ //经济模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"24FF"];
        }else if (instance.heating_select_model == custom){ //自定义模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"25FF"];
        }
    }else{ //热水
        if (instance.hotwater_select_model == convention) { //常规模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"11FF"];
        }else if (instance.hotwater_select_model == comfortable){ //舒适模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"12FF"];
        }
    }
}
...
+(void)sendSettingTempretureCmd:(SkywareDeviceInfoModel *)skywareInfo
{
    ...
    if (instance.deviceFunState == heating_fun) { //采暖
        if (instance.heating_select_model == ceaseless_run) { //无定时模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"21%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }else if (instance.heating_select_model == business_one){ //商务模式1
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"22%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }else if (instance.heating_select_model == business_two){ //商务模式2
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"23%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }else if (instance.heating_select_model == economy){ //经济模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"24%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }
    }else{ //热水
        if (instance.hotwater_select_model == convention) { //常规模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"11%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }else if (instance.hotwater_select_model == comfortable){ //舒适模式
            [SkywareDeviceManagement DevicePushCMDWithEncodeData:[NSString stringWithFormat:@"12%@",[UtilConversion decimalToHex:instance.defaultTem]]];
        }
    }
}
...
+(void)sendSeasonChangeCmd:(SkywareDeviceInfoModel *)skywareInfo
{
    ...
    if (data.seasonWinter) { //冬季
        NSLog(@"冬季2001");
        [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"2001"];
    }else{//夏季
        NSLog(@"夏季2001");
        [SkywareDeviceManagement DevicePushCMDWithEncodeData:@"2002"];
    }
}
